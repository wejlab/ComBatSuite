...
#' create surrogate variables which are then used to remove batch effects both
#' from the training database and a new data set for prediction purposes. For
#' inferential analysis see \code{\link{sva}}, \code{\link{svaseq}}, with low
#' level functionality available through \code{\link{irwsva.build}} and
#' \code{\link{ssva}}.
#' 
#' @param dbdat A m genes by n arrays matrix of expression data from the
#'   database/training data
#' @param mod The model matrix for the terms included in the analysis for the
#'   training data
#' @param sv The surrogate variable object created by running sva on dbdat
#'   using mod. 
#' @param newdat (optional) A set of test samples to be adjusted using the
#'   training database
#' @param method If method ="fast" then the SVD is calculated using an online
#'   approach, this may introduce slight bias. If method="exact" the exact SVD
...
fsva <- function(dbdat, mod, sv, newdat=NULL, method=c("fast","exact")){
    ...
    if(!is.null(newdat)){
        ...
        if(method=="fast"){
            ...
            D <- svd.wx$d[seq_len(n.sv)]
            U <- svd.wx$u[,seq_len(n.sv), drop=FALSE]
            P <- t(wts*t(1/D * t(U)))
            newV <- P %*% newdat
            sgn <- rep(NA,n.sv)
