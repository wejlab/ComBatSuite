...
#' This function first applies a moderated log transform as described in Leek
#' 2014 before calculating the surrogate variables. See the function
...
#' @param controls A vector of probabilities (between 0 and 1, inclusive) that
...
#' @param method For empirical estimation of control probes use "irw". If
...
#' @param numSVmethod If n.sv is NULL, sva will attempt to estimate the number
#'   of needed surrogate variables. This should not be adapted by the user
#'   unless they are an expert. 
#' @param constant The function takes log(dat + constant) before performing sva.
#'   By default constant = 1, all values of dat + constant should be positive. 
#' 
...
svaseq <- function(dat, mod, mod0 = NULL,n.sv=NULL,controls=NULL,
