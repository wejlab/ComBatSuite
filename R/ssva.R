...
#' biological variables of interest are assumed to be known in advance. This
#' supervised sva approach can be called through the \code{\link{sva}} and
#' \code{\link{svaseq}} functions by specifying controls. 
#' 
...
ssva <- function(dat,controls,n.sv){
    ...
    sv <- ss$v[,seq_len(n.sv), drop=FALSE]
